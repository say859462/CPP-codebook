...
// freopen("output.txt","w",stdout);  // 寫入 file.out 檔
    ...
    while(rec[sz-1]<=5e10){
        for(int i=0;i<2;i++){
            for(int j=qpow(2,use)-2,cnt=0;cnt<qpow(2,use);cnt++,j++){
                ll tmp=num[i]*qpow(10,use)+rec[j];
                rec.pb(tmp);
                sz++;
            }
        }
    ...
    ll pre=l;
    ll end=r;
    l=lower_bound(ALL(rec),l)-rec.begin();
    r=lower_bound(ALL(rec),r)-rec.begin();
    if(l==r){
        cout<<(end-pre+1)*rec[l]<<endl;
        return 0;
    }
    ans+=rec[l]*(rec[l]-pre+1);
    for(int i=l+1;i<=r-1;i++){
        ans+=rec[i]*(rec[i]-rec[i-1]);
    }
    ans+=rec[r]*(end-rec[r-1]);
    cout<<ans<<endl;
    return 0;
}
/*
