...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        for(int j=0;j<n;j++){
            if(i==j)continue;
            if(!ok[i] && !ok[j] && (rec[i]+rec[j])%2==0 && (rec[i]+rec[j])/2==avg){
                ok[i]=ok[j]=1;
                if(rec[i]<rec[j])ans.PB(node{i,j,rec[j]-avg});
                else ans.PB(node{j,i,rec[i]-avg});
            }
        }
    ...
    for(int i=0;i<SZ(ans);i++){
        cout<<ans[i].w<<" ml. from cup #"<<ans[i].u+1<<" to cup #"<<ans[i].v+1<<"."<<endl;
    }
    return ;
}
int main(){
