...
int n, m, z, t, w, u, h, v, x, y, k, l, r, d;
struct node
{
    int pre, cur, total;
    node(){};
    node(int _pre, int _cur, int _total)
    {
        pre = _pre;
        cur = _cur;
        total = _total;
...
int gem[30005] = {0};
int dp[30005][550] = {0}; // dp[i][j] 當前為i島 且上一次跳躍距離為j-offset長度
void solve()
{
    cin >> n >> d;
    for (int i = 0; i < n; i++)
    ...
    for (int i = d; i <= 30000; i++)
    {
        for (int j = 1; j <= 500; j++)
        {
            if (dp[i][j] == -1)
                continue;
            int dis = i + j + d - 250; //跳躍距離[d-250,d+250] + 上次跳躍距離j
            if (dis <= 30000)
            {
                dp[dis][j] = max(dp[dis][j], dp[i][j] + gem[dis]);
                ans = max(ans, dp[dis][j]);
            }
            if (dis + 1 <= 30000)
            {
                dp[dis + 1][j + 1] = max(dp[dis + 1][j + 1], dp[i][j] + gem[dis + 1]);
                ans = max(ans, dp[dis + 1][j + 1]);
            }
...
/*
假設一開始跳躍d距離 之後都選擇d+1的跳躍模式 則跳躍距離累加為d+(d+1)+(d+2)+...+(d+250)>3w 也就是說 跳躍距離只會在區間[d-250,d+250]的範圍內。
dp[i][j]為在島i上且前一次跳躍距離為j的最大gem數
跳躍距離計算為 (d+j)-250 前一次跳躍距離+d+偏移量
*/
