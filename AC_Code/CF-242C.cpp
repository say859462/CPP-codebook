...
{
    if (dx >= 1 && dx <= int(1e9) && dy >= 1 && dy <= int(1e9) && mp[MP(dx, dy)] && !vis[MP(dx, dy)])
        return true;
    return false;
}
void solve()
{
    int st_st, st_ed, ed_st, ed_ed;
    cin >> st_st >> st_ed >> ed_st >> ed_ed;
    cin >> n;
    for (int i = 0; i < n; i++)
    {
        cin >> r >> x >> y;
    ...
    while (!q.empty())
    {
        ...
        if (pos.first == ed_st && pos.second == ed_ed)
        {
            ans = min(ans, step);
            break;
        }
        ...
        for (int i = 0; i < 8; i++)
        {
            int dx = pos.first + mov[i][0];
            int dy = pos.second + mov[i][1];
            if (check(dx, dy))
            {
                vis[MP(dx, dy)] = 1;
