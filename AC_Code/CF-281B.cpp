...
int main(){
   ...
   //freopen("output.txt","w",stdout);  // 寫入 file.out 檔
    ...
    double tmp=(x*1.0) / (y*1.0);
    int ans1=0,ans2=1;
    for(int i=1;i<=n;i++){
        int l=0,r=100000; 
        while(l<=r){
            int mid=(l+r)>>1;
            if(mid*1.0/ (i*1.0)>=tmp)r=mid-1;
            else l=mid+1;
        }
        if(abs((l-1)*1.0/(i*1.0)-tmp)<abs((ans1*1.0)/(ans2*1.0)-tmp))
            ans1 = l-1,ans2 = i;
        if( abs( (l*1.0) /(i*1.0)-tmp) <abs((ans1*1.0)/(ans2*1.0)-tmp) ){
            ans1=l,ans2=i;
        }
    }
    c=gcd(ans1,ans2);
