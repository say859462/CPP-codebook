...
int n,m,z,t,w,x,y,v,c,k,u;
int same[105]={0},decrease[105]={0},other[105]={0};
int main(){
  ...
  //  freopen("output.txt","w",stdout);  // 寫入 file.out 檔
    ...
    for(int i=0;i<n;i++){
        ...
        while(k--){
            cin>>phone;
            if(phone[0]==phone[1] && phone[1]==phone[3] && phone[3]==phone[4] && phone[4]==phone[6] && phone[6]==phone[7])same[i]++;
            else if(phone[0] > phone[1] && phone[1] > phone[3] && phone[3] > phone[4] && phone[4]>phone[6] && phone[6]>phone[7])decrease[i]++;
            else other[i]++;
        }
        m1=max(m1,same[i]);
        m2=max(m2,decrease[i]);
        m3=max(m3,other[i]);
    }
