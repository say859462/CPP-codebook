...
void update_replace(int idx,int l,int r,int pos,ll val){
    push_down(idx,l,r);
    if(l==pos && r==pos){   
        tree[idx]=val;
        return ;
    ...
    int mid=(l+r)>>1;
    if(pos<=mid)update_replace(cl(idx),l,mid,pos,val);
    else update_replace(cr(idx),mid+1,r,pos,val);
    return ;
}
void update_add(int idx,int l,int r,int ql,int qr,ll val){
    push_down(idx,l,r);
    if(ql<=l && r<=qr){
        tag[idx]+=val;
        tree[idx]+=val;
    ...
    int mid=(l+r)>>1;
    if(ql<=mid)update_add(cl(idx),l,mid,ql,qr,val);
    if(qr>mid)update_add(cr(idx),mid+1,r,ql,qr,val);
...
ll query(int idx,int l,int r,int pos){
    ...
    int mid=(l+r)>>1;
    if(pos<=mid)return query(cl(idx),l,mid,pos);
    else return query(cr(idx),mid+1,r,pos);
}
int  main(){   
...
// freopen("output.txt","w",stdout);  // 寫入 file.out 檔
    ...
    while(q--){
        ...
        if(k==1){//replace
            cin>>l>>r;
            update_replace(1,1,n,l,r);
        }
        else if(k==2){//add all
            cin>>x;
            update_add(1,1,n,1,n,x);
        }
        else{//query
            cin>>x;
