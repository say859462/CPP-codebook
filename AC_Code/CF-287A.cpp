...
bool square(int x,int y,int x1,int y1,int x2,int y2,int x3,int y3){
    return (mp[x][y]==mp[x1][y1]&mp[x1][y1]==mp[x2][y2]&&mp[x2][y2]==mp[x3][y3]);
}
bool check(int x,int y){
    for(int i=0;i<4;i++){
        int x1=x,y1=y,x2=x,y2=y,x3=x,y3=y;
        x1+=mov[i][0][0],y1+=mov[i][0][1];
        x2+=mov[i][1][0],y2+=mov[i][1][1];
        x3+=mov[i][2][0],y3+=mov[i][2][1];
        if(in(x1)&&in(y1)&&in(x2)&&in(y2)&&in(x3)&&in(y3)){
            if(square(x,y,x1,y1,x2,y2,x3,y3)){
                return true;
            }
        }
    }
