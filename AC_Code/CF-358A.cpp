...
int main()
{
    ...
    for(int i=0;i<n-1;i++){
        int xx=rec[i],yy=rec[i+1];
        if(xx>yy)swap(xx,yy);
        pt.pb(make_pair(xx,yy));
    }
    for(int i=0;i<SZ(pt)-1;i++){
        for(int j=i+1;j<SZ(pt);j++){
            if( (pt[i].first < pt[j].first &&pt[j].first <pt[i].second && pt[j].second >pt[i].second ) || (pt[j].first<pt[i].first &&pt[i].first < pt[j].second &&pt[i].second >pt[j].second)){
                cout<<"yes"<<endl;
                return 0;
            } 
        }
