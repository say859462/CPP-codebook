...
int main()
{
    ...
    for(int i=1;i<=n;i++)
        for(int j=1;j<=m;j++)
            cin>>num[i][j];
   // for(int i=0;i<SZ(prime);i++)cout<<prime[i]<<" ";
    ll mn=INFLL;
    for(int i=1;i<=n;i++){//enum row
        ll tmp=0;
        for(int j=1;j<=m;j++){
            int pos=lower_bound(ALL(prime),num[i][j])-prime.begin();
            
            if(prime[pos]==num[i][j])continue;
            tmp+=prime[pos]-num[i][j];
        }
        mn=min(mn,tmp);
    }
    for(int i=1;i<=m;i++){//enum col
        ll tmp=0;
        for(int j=1;j<=n;j++){
            int pos=lower_bound(ALL(prime),num[j][i])-prime.begin();
          //  cout<<prime[pos]<<endl;
            if(prime[pos]==num[j][i])continue;
            tmp+=prime[pos]-num[j][i];
        }
        mn=min(mn,tmp);
    }
    cout<<mn<<endl;
