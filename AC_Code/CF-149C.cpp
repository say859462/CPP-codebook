...
void solve(){
    ...
    while(sum1-sum2>mx && abs(SZ(t1)-SZ(t2))<=1){
        int point=t1[0].first;
        int idx=t1[0].second;
        t1.erase(t1.begin());
        sum1-=point;
        sum2+=point;
    ...
    cout<<SZ(t1)<<endl;
    for(int i=0;i<SZ(t1);i++)cout<<t1[i].second<<" ";
    cout<<endl<<SZ(t2)<<endl;
    for(int i=0;i<SZ(t2);i++)cout<<t2[i].second<<" ";
    cout<<endl;
    return ;
}
int main(){
