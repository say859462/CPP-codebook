...
int n,m,z,t,w,x,y,k,u,h;
double num[200][200]={0};
double ans[200][200]={0};
double ans2[200][200]={0};
void solve(){
    ...
    for(int i=1;i<=n;i++){
        for(int j=1;j<=n;j++){
            if(i==j)continue;
            ans[i][j]=(num[i][j]+num[j][i])/2.0;
            ans2[i][j]=num[i][j]-ans[i][j];
        }
    }
    for(int i=1;i<=n;i++){
        for(int j=1;j<=n;j++){
    ...
    for(int i=1;i<=n;i++){
        for(int j=1;j<=n;j++){
            cout<<fixed<<setprecision(10)<<ans2[i][j]<<" ";
        }
        cout<<endl;
    }
    return;
