...
ll n, m, z, t, w, u, h, d, v, x, y, k, l, r;
ll pre[1000050] = {0};
map<ll, ll> cnt;
void solve()
{
    ...
    if (n == 0)
    {
        cout << cnt[0] * SZ(s) * (SZ(s) + 1) - cnt[0] * cnt[0] << endl;
        return;
    }
    for (int i = 1; i <= SZ(s); i++)
    {
        for (int j = 1; j <= i; j++)
        {
            ll tmp = pre[i] - pre[j - 1];
            if (tmp != 0 && n % tmp == 0 && n / tmp < 1000100)
            {
                ans += cnt[n / tmp];
            }
        }
...
/*
觀察到每個子矩陣的和為(sum[x2]-sum[x1-1])*(sum[y2]-sum[y1-1])
故預處理每行各段總和，之後再枚舉每列去配對是否可以構成總和n即可

但要特判總合為0的情況，當字串s[i]==0時，第i行/列均為0，對於i行可以構成n*(n+1)/2個子矩陣，所以可以跟i列構成2* n*(n+1)/2 個子矩陣，但是a[i][i]會被重複計算 須扣除
*/
int main()
{
