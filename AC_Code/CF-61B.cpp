...
void solve(){
    ...
    for(int i=0;i<SZ(a);i++){
        if(a[i]==';' || a[i]=='_' || a[i]=='-'){
            while(a[i]==';' || a[i]=='-' || a[i]=='_' )a.erase(i,1); 
        }
    }
    ...
    for(int i=0;i<SZ(b);i++){
        if(b[i]==';' || b[i]=='_' || b[i]=='-'){
            while(b[i]==';' || b[i]=='-' || b[i]=='_' )b.erase(i,1); 
        }
    }
    for(int i=0;i<SZ(c);i++){
        if(c[i]>='A'&&c[i]<='Z')c[i]=tolower(c[i]);
    }
    for(int i=0;i<SZ(c);i++){
       if(c[i]==';' || c[i]=='_' || c[i]=='-'){
            while(c[i]==';' || c[i]=='-' || c[i]=='_' )c.erase(i,1); 
        }
    }
    mp[a+b+c]=1,mp[a+c+b]=1,mp[b+a+c]=1,mp[b+c+a]=1,mp[c+a+b]=1,mp[c+b+a]=1;
    int q;
    cin>>q;
    while(q--){
        cin>>s;
        ...
        for(int i=0;i<SZ(s);i++){
            if(s[i]=='_' || s[i]=='-' || s[i]==';'){
                while(s[i]=='-' || s[i]=='_' || s[i]==';')s.erase(i,1);
            }
        }
        if(mp[s])cout<<"ACC"<<endl;
        else cout<<"WA"<<endl;
