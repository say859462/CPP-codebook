...
int n, m, z, t, w, u, h, d, v, x, y, k, l, r;
int ans = INF;
struct node
{
    int x, y, a, b;
...
bool check()
{
    ...
    sort(p, p + 4, cmp);
    int s1, s2, s3, s4, k;
    s1 = (p[0].x - p[1].x) * (p[0].x - p[1].x) + (p[1].y - p[0].y) * (p[1].y - p[0].y);
    s2 = (p[0].x - p[2].x) * (p[0].x - p[2].x) + (p[2].y - p[0].y) * (p[2].y - p[0].y);
    s3 = (p[3].x - p[1].x) * (p[3].x - p[1].x) + (p[3].y - p[1].y) * (p[3].y - p[1].y);
    s4 = (p[3].x - p[2].x) * (p[3].x - p[2].x) + (p[3].y - p[2].y) * (p[3].y - p[2].y);
    k = (p[2].x - p[0].x) * (p[1].x - p[0].x) + (p[2].y - p[0].y) * (p[1].y - p[0].y);
...
void dfs(int cur, int res)
{
    ...
    node t = rec[cur], tt = rec[cur];
    for (int i = 0; i < 4; i++)
    {
        int dx = t.x, dy = t.y, da = t.a, db = t.b;
        if (i > 0)
        {
            t.x = -(dy - db) + da;
            t.y = (dx - da) + db;
...
void solve()
{
    ...
    while (n--)
    {
        for (int i = 0; i < 4; i++)
        {
            cin >> rec[i].x >> rec[i].y >> rec[i].a >> rec[i].b;
        }
        ans = INF;
        dfs(0, 0);
        if (ans == INF)
