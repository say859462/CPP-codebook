...
void solve(){
    ...
    if(n==2){
        if(rec[0]==rec[1]){
            ...
            if(d%2){
                cout<<2<<endl;
                cout<<rec[0]-d<<" "<<rec[1]+d<<endl;;
            }
            else{
                cout<<3<<endl;
                cout<<rec[0]-d<<" "<<rec[0]+d/2<<" "<<rec[1]+d<<endl;
            }
        }
        else{
            cout<<2<<endl;
            cout<<rec[0]-1<<" "<<rec[1]+1;
        }
        return ;
    }
    ...
    //1.already is 2.need 1 3.cant construct
    set<ll>s;
    bool flag=1;
    for(int i=0;i<n-1;i++){
        s.insert(rec[i+1]-rec[i]);
        if(rec[i+1]!=rec[i]){
            flag=0;
        }
    }
    ...
    if(SZ(s)==1){
        ll d=rec[1]-rec[0];
        cout<<2<<endl;
        cout<<rec[0]-d<<" "<<rec[n-1]+d<<endl;
    }
    else if(SZ(s)==2){
        ll num1=0,num2=0;
        map<ll,int>tmp;
        for(int i=0;i<n-1;i++){
            if(!tmp[rec[i+1]-rec[i]] && !num1){
                num1=rec[i+1]-rec[i];
            }
            else if(!num2 && !tmp[rec[i+1]-rec[i]]){
                num2=rec[i+1]-rec[i];
            }
            tmp[rec[i+1]-rec[i]]++;
        }
        if(tmp[num1]>1 && tmp[num2]>1){
            cout<<0<<endl;
        }
        else{
            if(num1>num2)swap(num1,num2);
            if(tmp[num2]>tmp[num1]){
                cout<<0<<endl;
            }
            else{
                if(num2%2==0 && num2/2==num1){
                    cout<<1<<endl;
                    for(int i=0;i<n-1;i++){
                        if(rec[i+1]-rec[i]!=num1){
                            cout<<rec[i]+num1<<endl;
                            break;
                        }
                    }
                }
