...
ll n,m,a,b,c,d,z,x,y,k,t,w;
char cmd;
const int mx=1e6+5;
int tree[4*mx]={0};
int num[mx]={0};
void build(int idx,int l,int r,int mode){
    if(l==r){
        tree[idx]=num[l];
        return ;
    }
    ...
    build(cl(idx),l,mid,mode^1);
    build(cr(idx),mid+1,r,mode^1); 
    if(mode){
...
void update(int idx,int l,int r,int pos,int val,int mode){
    if(l==r){
        tree[idx]=val;
        return ;
    }
    int mid=(l+r)>>1;
    if(pos<=mid)update(cl(idx),l,mid,pos,val,mode^1);
    else update(cr(idx),mid+1,r,pos,val,mode^1);
    if(mode){
        tree[idx]=tree[cl(idx)]|tree[cr(idx)];
    }
    else{
...
int main(){
    ...
    int mode=n%2?1:0;// 1 or 0 xor
    build(1,1,sz,mode);
    
    while(m--){
        cin>>x>>y;
        update(1,1,sz,x,y,mode);
        cout<<tree[1]<<endl;
    }
    return 0;
}
