...
bool check(string str){
    ...
    if( !number(str[0]) || !number(str[1]) || !number(str[3])  || !number(str[4]) || !number(str[6]) || !number(str[7]) || !number(str[8])  || !number(str[9]) )
        return false;
    int day=str[1]-'0' + 10*(str[0]-'0');
    int mon=str[4]-'0'+ 10 * (str[3]-'0');
    int year=(str[6]-'0')*1000+(str[7]-'0')*100+(str[8]-'0')*10+(str[9]-'0');
   // cout<<day<<" "<<mon<<" "<<year<<endl;
    if(mon>12 || mon < 1)return false;
    if(day>month[mon] || day<1)return false;
...
int main(){
   ...
   //freopen("output.txt","w",stdout);  // 寫入 file.out 檔
    ...
    pair<string,int>ans;
    map<string,int>cnt;
    for(int i=0;i+10-1<SZ(s);i++){
        string tmp=s.substr(i,10);
        if(check(tmp)){
            cnt[tmp]++;
            if(cnt[tmp]>ans.second){
                ans.second=cnt[tmp];
                ans.first=tmp;
            }
        }
