...
void push(int idx,int l,int r){
    if(tag[idx]){
        ...
        tag[cl(idx)]+=tag[idx];
        tag[cr(idx)]+=tag[idx];
        tree[cl(idx)]+=(mid-l+1)*tag[idx];
        tree[cr(idx)]+=(r-mid)*tag[idx];
        tag[idx]=0;
    }
    return ;
}
void update(int idx,int l,int r,int ql,int qr,int val,bool mode){
    if(ql<= l  && r <= qr){
        if(mode){
            tree[idx]=val;
        }
    ...
    push(idx,l,r);
    int mid=(l+r)>>1;
    if(ql<=mid)update(cl(idx),l,mid,ql,qr,val,mode);
    if(qr>mid)update(cr(idx),mid+1,r,ql,qr,val,mode);
    tree[idx]=tree[cl(idx)]+tree[cr(idx)];
    return ;
}
ll n,m,z,t,w,k,u,h,d,v,x,y;
...
void solve(){
    ...
    while(n--){
        ...
        if(k==1){
            cin>>x>>y;
            update(1,1,mx,1,x,y,0);
        }
        else if(k==2){
            cin>>x;
            sz++;
            update(1,1,mx,sz,sz,x,1);
        }
        else{
            if(sz<2)continue;
            update(1,1,mx,sz,sz,0,1);
            sz--;
        }
       cout<<fixed<<setprecision(10)<<double(tree[1])/sz*1.0<<endl;
    }
    return ;
}
int main(){
