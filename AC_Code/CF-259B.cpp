...
ll n,m,z,k,t,w,x,y,v,c;
int num[4][4]={0};
const int mx=3e5+5;
bool check(){
    int r1=0,r2=0,r3=0;
    for(int i=1;i<=3;i++)r1+=num[1][i];
    for(int i=1;i<=3;i++)r2+=num[2][i];
    for(int i=1;i<=3;i++)r3+=num[3][i];
    if(r1!=r2 || r2!=r3 || r1!=r3)return false;
    int c1=0,c2=0,c3=0;
    for(int i=1;i<=3;i++)c1+=num[i][1];
    for(int i=1;i<=3;i++)c2+=num[i][2];
    for(int i=1;i<=3;i++)c3+=num[i][3];
    if(c1 != c2 || c2!= c3 || c1!=c3)return false;
    int d1=0,d2=0;
    d1=num[1][1]+num[2][2]+num[3][3];
    d2=num[1][3]+num[2][2]+num[3][1];
   ...
   // freopen("output.txt","w",stdout);  // 寫入 file.out 檔
    ...
    for(int i=1;i<=3;i++)
        for(int j=1;j<=3;j++)
            cin>>num[i][j];
    for(int i=1;i<=mx;i++){
        num[1][1]=i-(num[1][2]+num[1][3]);
        num[2][2]=i-(num[2][1]+num[2][3]);
        num[3][3]=i-(num[1][1]+num[2][2]);
        if(num[1][1]< 0 || num[2][2] < 0 || num[3][3] <0)continue;
        if(check()){
            for(int i=1;i<=3;i++){
                for(int j=1;j<=3;j++){
                    cout<<num[i][j]<<" ";
