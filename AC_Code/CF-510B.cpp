    ...
    for (int i = 1; i <= n; i++)
    {
        for (int j = 1; j <= m; j++)
        {
            for (int k = 0; k < 2; k++)
            {
                int dx = i + mov[k][0];
                int dy = j + mov[k][1];
                if (dx >= 1 && dx <= n && dy >= 1 && dy <= m && mp[i][j] == mp[dx][dy])
                {
                    edge[id[i][j]].PB(id[dx][dy]);
                    edge[id[dx][dy]].PB(id[i][j]);
                }
            }
        }
    }
...
/*
key point:if cycle exist dfs/bfs must visit the node which is already visited
*/
