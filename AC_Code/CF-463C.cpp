...
int n,m,z,t,w,k,u,h;
ll mp[2005][2005]={0};
ll diag1[10000]={0},diag2[10000]={0};
void solve(){
    cin>>n;
    int offset=n;
    for(int i=1;i<=n;i++)
    ...
    for(int i=1;i<=n;i++){
        for(int j=1;j<=n;j++){
            if((i+j)&1){
                if(mx1<diag1[i+j]+diag2[i-j+offset]-mp[i][j]){
                    mx1=diag1[i+j]+diag2[i-j+offset]-mp[i][j];
                    x1=i,y1=j;
                }
            }
            else{
                if(mx2<diag1[i+j]+diag2[i-j+offset]-mp[i][j]){
                    mx2=diag1[i+j]+diag2[i-j+offset]-mp[i][j];
                    x2=i,y2=j;
                }
            }
        }
