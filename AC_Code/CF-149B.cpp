...
#define dsort(a) sort(a.begin(), a.end(), greater<int>())  //降冪排序             //升冪排序
#define dsortll(a) sort(a.begin(), a.end(), greater<ll>()) //降冪排序
const int mod=1e6+3;
ll gcd(ll a,ll b){//輾轉相除法
    while(b){
        ll tmp=b;
        b=a%b;
        a=tmp;
...
int n,m,z,t,w,x,y,v,c,k,u,start;
int a[10]={0},b[10]={0};
string hour,minute;
bool check(int mid){
    int sum1=0,sum2=0;
    int base=1;
    for(int i=SZ(hour)-1;i>=0;i--){
        sum1+=base*a[i];
        base*=mid;
    }
    base=1;
    for(int i=SZ(minute)-1;i>=0;i--){
        sum2+=base*b[i];
        base*=mid;
    }
    if(sum1>=24 || sum2>=60)return false;
    return true;
}
int change(char c){
...
void solve(){
    ...
    for(int i=0;i<SZ(s);i++){
        ...
        mx=max(mx,change(s[i]));
    }
    hour=s.substr(0,pos);
    minute=s.substr(pos+1,SZ(s)-pos);
    while(hour[0]=='0' && SZ(hour))hour.erase(0,1);
    while(minute[0]=='0' && SZ(minute))minute.erase(0,1);

    for(int i=0;i<SZ(hour);i++){
        if(hour[i]>='0' && hour[i]<='9')a[i]=hour[i]-'0';
        else a[i]=hour[i]-'A'+10;
    }
    for(int i=0;i<SZ(minute);i++){
        if(minute[i]>='0' && minute[i]<='9')b[i]=minute[i]-'0';
        else b[i]=minute[i]-'A'+10;
    }
   // cout<<hour<<" "<<minute<<endl;
    if( SZ(hour)<2 && a[SZ(hour)-1]<24 && SZ(minute)<2 && b[SZ(minute)-1]<60 ){
        cout<<-1<<endl;
        return ;
    }
    vi ans;
