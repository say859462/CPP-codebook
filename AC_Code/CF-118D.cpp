...
int main(){
    ...
    IO_FAST;
    cin>>a>>b>>c>>d;
    ll dp[105][105][2]={0};//dp[i][j] i個步兵 j個騎兵 結尾為0 步兵 ,1 騎兵的方案數
    for(int i=0;i<=a && i<=c;i++)dp[i][0][0]=1;
    for(int i=0;i<=b && i<=d;i++)dp[0][i][1]=1;
    for(int i=1;i<=a;i++){//當前有i+j個士兵
        for(int j=1;j<=b;j++){
            for(int k=1;k<=c && k<=i;k++){//加入步兵 0
                dp[i][j][0]+=dp[i-k][j][1];
                dp[i][j][0]%=mod;
            }
