...
struct node
{
    int hp, atk, def;
    node() {}
    node(int _hp, int _atk, int _def)
    {
        hp = _hp;
        atk = _atk;
        def = _def;
...
{
    int ans = INF;
    cin >> p.hp >> p.atk >> p.def;
    cin >> monster.hp >> monster.atk >> monster.def;
    cin >> price.hp >> price.atk >> price.def;
    for (int i = p.atk; i <= 201; i++)
    {
        for (int j = p.def; j <= 100; j++)
        {
            int rec = (i - p.atk) * price.atk + (j - p.def) * price.def;
            int damage = i - monster.def;
            int damage2 = monster.atk - j;
            int hp1 = p.hp, hp2 = monster.hp;
            if (damage <= 0)
                continue;
            while (hp2 > 0 && damage2 >= 0)
            {
                hp1 -= damage2;
                hp2 -= damage;
                if (hp1 <= 0)
