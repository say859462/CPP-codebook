...
bool check(int num) // O(logn)
{
    while (num)
    {
        if (num % 10 != x && num % 10 != y)
            return false;
...
void solve()
{
    build();
    cin >> x >> y >> n;
    for (int i = 0; i <= n; i++) //枚舉x有i個
    {
        if (check(i * x + (n - i) * y))
        {
            ans = ans + ((fac[n] * inv[i]) % mod * inv[n - i]) % mod;
            ans %= mod;
        }
    }
    cout << ans % mod << endl;
...
/*
學習:模逆元的運用(費馬小定理)，若算式為a/b，則其取模的結果應當為a*b^-1，b^-1為b的模逆元，可由費馬小定理得到，a^(p-1)=1(mod p) if gcd(a,p)=1 =>a^(p-2)=a^-1(mod p)
b^-1=qpow(b,mod-2);  
*/
