...
int main()
{
    ...
    for(int i=0;i<SZ(num);i++){
        ...
        if(SZ(pos[num[i]])==1){
            t++;
            ans.pb(make_pair(num[i],0));
            continue;
        }
        int d=pos[num[i]][1]-pos[num[i]][0];
        for(int j=1;j<SZ(pos[num[i]])-1;j++){
            if(pos[num[i]][j]+d!=pos[num[i]][j+1]){
                flag=0;
                break;
            }
        }
        if(flag){
            t++;
            ans.pb(make_pair(num[i],pos[num[i]][1]-pos[num[i]][0]));
        }
    }
    cout<<t<<endl;
    for(int i=0;i<t;i++){
