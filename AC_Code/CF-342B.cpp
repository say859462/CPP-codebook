...
int main(){
   ...
   //freopen("output.txt","w",stdout);  // 寫入 file.out 檔
    ...
    for(int i=0;i<m;i++){
        ...
        if(pos<f){
            if(time==rec[i].id){
                if(rec[i].l<=pos+1 && pos+1<=rec[i].r || rec[i].l<=pos && pos<=rec[i].r)ans.PB('X');
                else {
                    ans.PB('R');
                    pos++;
                }
                ...
                while(time++<rec[i].id && pos!=f){
                    ans.PB('R');
                    pos++;
                }
                if(pos==f)break;
                if(rec[i].l<=pos+1 && pos+1<=rec[i].r || rec[i].l<=pos && pos<=rec[i].r ){
                    ans.PB('X');
                }
                else {
                    ans.PB('R');
    ...
    if(pos!=f){
        if(pos>f){
            while(pos-->f)ans.PB('L');
        }
        else{
            while(pos++<f)ans.PB('R');
        }
    }
    cout<<ans<<endl;
    return 0;   
