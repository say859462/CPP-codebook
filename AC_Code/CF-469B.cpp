...
int main(){
  ...
  // freopen("input.txt","r",stdin);    // 讀 file.in 檔
   // freopen("output.txt","w",stdout);  // 寫入 file.out 檔
    ...
    for(int i=0;i<m;i++)cin>>rec2[i].first>>rec2[i].second;
    int ans=0;
    for(int i=l;i<=r;i++){
        bool flag=0;
        for(int j=0;j<m;j++){
            int dx=rec2[j].first+i;
            int dy=rec2[j].second+i;
            for(int k=0;k<n;k++){
                if(rec[k].first<=dx&&dx<=rec[k].second || rec[k].first<=dy&&dy<=rec[k].second ||dx<=rec[k].first && rec[k].first<=dy ||dx<=rec[k].second&&rec[k].second<=dy ){
                    flag=1;
                    break;
                }
            }
