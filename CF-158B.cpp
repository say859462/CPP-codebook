...
#include<bits/stdc++.h>
using namespace std;
#define IO_FAST cin.tie(0);ios::sync_with_stdio(0)
#define ll long long
#define LL long long
...
int main(){
    ...
    int n,x;
    cin>>n;
    for(int i=1;i<=n;i++)cin>>x,rec[x]++;
    int ans=0;
    ans+=rec[4];
    rec[4]=0;
    int tmp=min(rec[3],rec[1]);
    ans+=tmp;
    rec[3]-=tmp;
    rec[1]-=tmp;
    ans+=rec[3];
    ...
    if(rec[1]&&rec[2])ans+=min(rec[1],rec[2]);
    tmp=min(rec[1],rec[2]);
    rec[1]-=tmp;
    rec[2]-=tmp;
    ans+=rec[2];
